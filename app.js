...
/* ── SUPABASE: GASTOS REALES ─────────────────────────────────
   Mantiene todos los valores/estructuras del dashboard.
   Solo reemplaza el libro de gastos si Supabase responde bien.
────────────────────────────────────────────────────────────── */
...
function normalizarFecha(fecha) {
  if (!fecha) return "—";
  // Supabase normalmente devuelve YYYY-MM-DD.
...
function refrescarTablaGastosSegunVista() {
...
async function cargarGastosDesdeSupabase() {
  if (typeof supabaseClient === "undefined") {
    console.warn("Supabase no está configurado. Se mantienen los datos locales del app.js.");
    return;
  }
  ...
  if (error) {
    ...
    console.warn("Se mantienen los datos locales del app.js.");
    return;
  }

  if (!data || data.length === 0) {
    console.warn("Supabase no devolvió registros. Se mantienen los datos locales del app.js.");
    return;
  }
  ...
  filteredDocs = [...docs];
  refrescarTablaGastosSegunVista();

  console.info(`Gastos cargados desde Supabase: ${docs.length}`);
}
...
async function handleFileUpload(event) {
  ...
  const fileGroup = ["jpg", "jpeg", "png"].includes(extension)
  ? "imagenes"
  ...
  if (uploadError) {
    ...
    alert("No se pudo subir el archivo a Supabase Storage. Revisa el bucket y las políticas.");
    event.target.value = "";
    return;
  }
  ...
  if (insertError) {
    ...
    alert("El archivo se subió, pero no se pudo crear el registro en la tabla.");
    event.target.value = "";
    return;
  }
  ...
  if (typeof cargarGastosDesdeSupabase === "function") {
    await cargarGastosDesdeSupabase();
...
function initDashboard() {
  ...
  // Carga los gastos reales desde Supabase sin borrar el resto del dashboard.
